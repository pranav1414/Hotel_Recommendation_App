...
def load_data():
    # Load the CSV file
    data = pd.read_csv('Hotel_Reviewss.csv')  # Corrected filename
    return data
...
def main():
    ...
    # Dropdown for hotel names
    st.markdown('**Select Your Hotel**')
    hotel_list = data['Hotel_Name'].unique().tolist()
    hotel_name = st.selectbox('', [''] + hotel_list, key='hotel_name')
    filtered_data = data if hotel_name == '' else data[data['Hotel_Name'] == hotel_name]

    # Filtering by 'Reviewer_Nationality' if applicable
